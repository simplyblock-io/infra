resource "aws_lambda_function" "instance_stopper" {
  ...
  role    = aws_iam_role.instance_stopper_lambda_exec_role.arn
  timeout = 120 # 2 mins
  environment {
    variables = {
...
resource "aws_iam_role" "instance_stopper_lambda_exec_role" {
...
resource "aws_iam_role_policy_attachment" "instance_stopper_policy_attach" {
  role       = aws_iam_role.instance_stopper_lambda_exec_role.name
